...
export default function ProductRatings({ item }) {
  ...
  if (!breed) return null; // guard

  const qualities = [
    'affection_level','child_friendly','stranger_friendly','dog_friendly',
    'adaptability','social_needs','health_issues','energy_level',
    'shedding_level','vocalisation'
  ];

  const formatQualityTitle = (stat) =>
    stat.replace('_', ' ').replace(/(^\w{1})|(\s+\w{1})/g, (m) => m.toUpperCase());
        ...
        {(breed.temperament || '')
          ...
          .map((t) => (
            <Stack key={t} direction="row" spacing={1}>
              <Chip label={t.trim()} variant="outlined" sx={{ alignSelf: 'stretch' }} />
            </Stack>
          ))}
      </Box>
      <br />
